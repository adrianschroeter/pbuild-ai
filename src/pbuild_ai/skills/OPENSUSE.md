...
- Prefer RPM macros in BuildRequires (e.g., %{python_module }) — they resolve against the build configuration that is always in effect: the distribution config, plus the project _config in project mode. Only stop using a macro when it cannot be used, which is detectable by the macro name itself appearing in the unresolvables (e.g., the literal unexpanded %{...} in the error). When the unresolvable names an expanded package instead — e.g., nothing provides python313-pipper — the macro works fine; fix the referenced package/module name and keep the macro (%{python_module pipper} → %{python_module pip}). Do NOT drop the macro or the BuildRequires line just because an expanded dependency is unresolvable, and never remove a whole dependency block because one entry is wrong. In project mode, prefer to fix a missing dependency by adding a further package to the project that provides it, rather than dropping the BuildRequires.
